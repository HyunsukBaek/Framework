...
_light1 attachTo [(vehicle player),[0, -0.3, -0.1]];
...
_speed = 30;// 가속도설정
_rockettype = "R_TBG32V_F";// 로켓 종류 기본값 M_NLAW_AT_F
_flaretype = "Sub_F_Signal_Green";	//로켓 부착 플레어 종류, Sub_F_Signal_Green, Sub_F_Signal_Red, CMflare_Chaff_Ammo
_boostcycle = 0;
...
while {_boostcycle < 40}do {
    ...
    if (_boostcycle == 5) then {
        ...
        flare = _flaretype createVehicle position _aircraft;
        flare attachTo [_aircraft, [0, 0.1, -0.1]];
