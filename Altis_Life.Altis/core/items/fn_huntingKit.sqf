...
if (_track select 2 >= 45 && _track select 2 < 135) then {_foundDirection = "to the Right";};
if (_track select 2 >= 135 && _track select 2 < 225) then {_foundDirection = "Behind";};
if (_track select 2 >= 225 && _track select 2 < 315) then {_foundDirection = "to the Left";};

if (_foundAnimal == "void") then {
	hint "No traces of animals found";
} else {
	hint format ["There are %1 traces of a %2 leading %3.",_foundDistance,_foundAnimal,_foundDirection];
};