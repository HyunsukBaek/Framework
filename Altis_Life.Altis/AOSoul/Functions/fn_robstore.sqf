...
_chance = random(100);
if(_chance >= 1) then {[1,format["ALARM! - Gas Station: %1 is being robbed!", _shop]] remoteExec ["life_fnc_broadcast",west]; };

if(_chance >= 1) then {[1,format["ALARM! - Gas Station: %1 is being robbed!", _shop]] remoteExec ["life_fnc_broadcast",civilian]; };
...
if(_rip) then
{
	...
	life_use_atm = true;
	if!(alive _robber) exitWith {};
	[getPlayerUID _robber,name _robber,"27"] remoteExec ["life_fnc_wantedAdd",2];
	//[getPlayerUID player,profileName,"27"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
};
[] spawn life_fnc_hudUpdate;
sleep 300;
